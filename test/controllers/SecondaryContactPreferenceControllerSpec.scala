  ...
  "SecondaryContactPreference Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val expectedJson = Json.obj(
        "form"       -> form,
        "mode"       -> NormalMode,
        "checkboxes" -> SecondaryContactPreference.checkboxes(form)
      )
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val filledForm = form.fill(SecondaryContactPreference.values.toSet)

      val expectedJson = Json.obj(
        "form"       -> filledForm,
        "mode"       -> NormalMode,
        "checkboxes" -> SecondaryContactPreference.checkboxes(filledForm)
      )
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val expectedJson = Json.obj(
        "form"       -> boundForm,
        "mode"       -> NormalMode,
        "checkboxes" -> SecondaryContactPreference.checkboxes(boundForm)
      )
    ...
    "must redirect to Session Expired for a POST if no existing data is found" in {
      ...
      val application = applicationBuilder(userAnswers = None).build()
      val request = FakeRequest(POST, secondaryContactPreferenceRoute).withFormUrlEncodedBody(("value[0]", SecondaryContactPreference.values.head.toString))
