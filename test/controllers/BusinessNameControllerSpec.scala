...
import models.{NormalMode, BusinessName, UserAnswers}
...
class BusinessNameControllerSpec extends SpecBase with MockitoSugar with NunjucksSupport with JsonMatchers {
  ...
  "BusinessName Controller" - {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, businessNameRoute)
          .withFormUrlEncodedBody(("businessName", "value 1"), ("field2", "value 2"))
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val application = applicationBuilder(userAnswers = Some(emptyUserAnswers)).build()
      val request = FakeRequest(POST, businessNameRoute).withFormUrlEncodedBody(("value", "invalid value"))
